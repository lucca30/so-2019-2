...
vector<int> AA_prioridade_dinamica;
    ...
    if(algoritmo_atual == PRIOd){
        ...
        if(AA_tarefa_em_execucao == -1 && !AA_heap.empty()){
            ...
            if(AA_ultima_tarefa_em_execucao != -1 && AA_tarefa_em_execucao != AA_ultima_tarefa_em_execucao){
                ...
                // atualizar as prioridades dinamicas
                set<pair<int, int> > new_heap;
                while(!AA_heap.empty()){
                    pair<int, int> temp = *(AA_heap.begin());
                    AA_heap.erase(AA_heap.begin());

                    temp.first -= 1;
                    new_heap.insert(temp);
                    printf("Atualizada a prioridade de %d para %d\n",-temp.second, -temp.first );
                }
                AA_heap = new_heap;
...
void inicia(void){
    ...
    for(int i=0;i<instante_chegada.size();i++){
        ...
        AA_prioridade_dinamica.pb(prioridades[i]);
